import Variant1Block1 from '@/components/variant-1/block-1/block-1';
import Variant1Block2 from '@/components/variant-1/block-2/block-2';
import Variant1Block3 from '@/components/variant-1/block-3/block-3';
import { LayoutVariantProps } from '@/types/layout.types';

/*
 * 1 manchete com foto, 2 destaques com foto, 6 destaques sem foto
 */
export function LayoutVariant1({ componentClass }: LayoutVariantProps) {
    return (
        <div className={`${componentClass} is-variant-1`}>
            <Variant1Block1 />
            <Variant1Block2 />
            <Variant1Block3 />
        </div>
    );
}
